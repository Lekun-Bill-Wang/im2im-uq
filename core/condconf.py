...
class CondConf:
    ...
    def predict(
        ...
        if exact:
            ...
            #print(f"Shape of x_test into prtial(_solve_dual,...): {x_test.shape}")
...
def _solve_dual(S, gcc, x_test, quantiles, threshold=None): # partial(_solve_dual, gcc=self, x_test=x_test, quantiles=quantiles, threshold=threshold)
    #print(f"Input S into _solve_dual(): {S}")
    #print(f"Shape of input x_test to _solve_dual(): {x_test.shape}")
    #print(f"{gcc.infinite_params}")
    if gcc.infinite_params.get('kernel', None):
        print("A kernel detected in _solve_dual()")
        prob = finish_dual_setup(
            gcc.cvx_problem,
            S,
            x_test,
        ...
        #rint("No kernel into _solve_dual()")
        S = np.concatenate([gcc.scores_calib, [S]], dtype=float)
        #print(f"Shape of scores_calib in _solve_dual(): {gcc.scores_calib.shape}") # for debugging
        #print(f"Shape of concatenated S in _solve_dual(): {S.shape}") # for debugging
        Phi = np.concatenate([gcc.phi_calib, gcc.Phi_fn(x_test)], axis=0, dtype=float)
        #print(f"Shape of concatenated Phi.T in _solve_dual(): {Phi.T.shape}") # for debugging
        zeros = np.zeros((Phi.shape[1],))
